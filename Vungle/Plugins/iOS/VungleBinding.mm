...
BOOL _vungleIsAdAvailable(const char* placementID) {
    return [VunglePluginAd canShow:GetStringParam(placementID)];
}

void _vungleLoadAd(const char* placementID) {
    [VunglePluginAd load:GetStringParam(placementID)];
}

void _vunglePlayAd(char* opt, const char* placementID) {
    [VunglePluginAd show:GetStringParam(placementID) options:GetStringParam(opt)];
}
...
void _updateCoppaStatus(BOOL isUserCoppa) {
    [[VungleSDK sharedSDK] updateCOPPAStatus:isUserCoppa];
}
